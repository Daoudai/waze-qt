...
#include "../websvc_trans/socket_async_receive.h"
#include "../websvc_trans/websvc_trans.h"
}

#define MAX_RETRIES 3

extern RNetworkManager* networkManager;
...
{
  memset( me, 0, sizeof(wst_context));

/* General:
   1  me->service              = NULL;
   2  me->content_type         = NULL;
   3  me->port                 = 0;              */
/* 4*/me->Socket               = ROADMAP_INVALID_SOCKET;/*
   5  me->state                = trans_idle;
   6  me->async_receive_started= FALSE;
   7  me->last_receive_time        = 0;              */
/* 8*/wstq_init(                 &(me->queue));  /*
   9  me->context              = NULL;           */
      me->retries              = 0;

/* Send:                                           */
/*10*/ebuffer_init(              &(me->packet));

/* Receive:                                        */
/*11*/cyclic_buffer_init(        &(me->CB));     /*
  12 me->http_parser_state     = http_not_parsed;
  13 me->parsers               = NULL;
  14 me->parsers_count         = 0;

  Completion:
  15 me->cbOnWSTCompleted      = NULL;
  16 me->result                = trans_succeeded;
  17 me->rc                    = succeeded;
  18 me->delete_on_idle        = FALSE;          */
   me->connect_context         = FALSE;
}

void wst_context_free( wst_context_ptr me)
{
   if( me->async_receive_started)
   {
      socket_async_receive_end( me->Socket);
      me->async_receive_started = FALSE;
   }

   if( ROADMAP_INVALID_SOCKET != me->Socket)
   {
      roadmap_net_close( me->Socket);
      me->Socket = ROADMAP_INVALID_SOCKET;
   }

   ebuffer_free( &(me->packet));
   wstq_clear  ( &(me->queue));
...
static void wst_context_reset( wst_context_ptr me, int use_ack)
{
// Don't loose these:
/* 1*/   const char*    service     = me->service;
/* 2*/   const char*    content_type= me->content_type;
/* 3*/   int            port        = me->port;
/* 4*/   RoadMapSocket  Socket      = me->Socket;
...
/* 6*/   me->async_receive_started= FALSE;
/* 7*/   me->last_receive_time        = 0;
// 8     me->queue    !NOT MODIFYING QUEUE!
/* 9*/   //me->context              = NULL;
   me->retries = 0;

/* Send:          */
/*10*/   ebuffer_free(              &(me->packet));

/* Receive:       */
/*11*/   cyclic_buffer_init(        &(me->CB));
/*12*/
   if (use_ack)
      me->http_parser_state    = http_not_acked;
   else
      me->http_parser_state    = http_not_parsed;
/*13*/   //me->parsers              = NULL;
/*14*/   //me->parsers_count        = 0;
...
/*17*/   me->rc                   = succeeded;
   ...
   me->active_item.type = WEBSVC_NO_TYPE;
   me->active_item.packet = NULL;

// Restore:
/* 1*/   me->service     = service;
/* 2*/   me->content_type= content_type;
/* 3*/   me->port        = port;
/* 4*/   me->Socket      = Socket;
}
...
static void  wst_context_load(wst_context_ptr         me,
                              int                     flags,
                              const char*             action,
                              int                     type,
                              const wst_parser_ptr    parsers,
                              ...
                              char*                   packet)
{
   wst_context_reset( me, (flags & WEBSVC_FLAG_V2));

//   me->action            = action;
//   me->parsers           = parsers;
//   me->parsers_count     = parsers_count;
//   me->cbOnWSTCompleted  = cbOnCompleted;
//   me->context           = context;
   me->state             = trans_active;

   //Save current item for retries
   wstq_item_init( &me->active_item);

   me->active_item.action        = action;
   me->active_item.type          = type;
   me->active_item.parsers       = parsers;
   me->active_item.parsers_count = parsers_count;
   me->active_item.cbOnCompleted = cbOnCompleted;
   me->active_item.context       = context;
   me->active_item.packet        = strdup(packet);
   me->active_item.flags         = flags;
}
                     ...
                     const char* content_type)  // e.g. - binary/octet-stream
{
   ...
   session = (wst_context_ptr) malloc( sizeof(wst_context));
...
void wst_term( wst_handle h)
{
   ...
   if( session)
   {
      ...
      {
         //assert(0);  // me IS NOT AN ERROR
                     // Just left me assert in order to see if me case happens...
...
// me method terminates the a-sync transaction:
// 1. Call callback
// 2. Reset context
                      ...
                      char*                packet)
{
   ...
   TQI.context       = context;
   TQI.packet        = strdup(packet); //AviR: Do we free me?
   TQI.flags         = flags;
...
// 1. me method is called from a single thread, thus no MT locks are used.
// 2. me method supports only one request at a time.
                     ...
                     ...)                                // Parameters
{
   ...
   // me parser will handle all data, which was not matched with
                                          ...
                                          roadmap_result    res)
{
   ...
   if( ROADMAP_INVALID_SOCKET == Socket)
   {
      ...
      /*
       * In me case the context is deallocated in the net layer.
       */
      session->connect_context = NULL;
      return trans_failed;
   }
...
void on_socket_connected( RoadMapSocket Socket, void* context, roadmap_result res)
{
   ...
   session->result = on_socket_connected_( Socket, (wst_context_ptr) context, res);
   ...
   while( CB->read_size > CB->read_processed )
   {
      ...
      if (session->http_parser_state != http_parse_completed) {
         //current request was removed from queue and replaced by a new request
          return (transaction_result) ((int) trans_canceled);
      }
...
{ memset( me, 0, sizeof(http_response_status));}

transaction_result http_response_status_load(
                              http_response_status*   me,
                              const char*             response,
                              BOOL                    verify_tag,
                              int*                    bytes_read)
{
